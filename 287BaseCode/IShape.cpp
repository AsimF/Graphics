...
//Change all to closed cylinder (add two disks)
...
IClosedCylinderY::IClosedCylinderY(const glm::vec3 & position, float R, float len):ICylinderY(position, R, len)
{
	topDisk = IDisk(position, Y_AXIS, R);
	bottomDisk = IDisk(position, Y_AXIS, R);
	topDisk.center.y += len / 2.0f;
	bottomDisk.center.y -= len / 2.0f;
}
...
void IClosedCylinderY::findClosestIntersection(const Ray &ray, HitRecord &hit) const {
	...
	for (int i = 0; i < numHits; i++) {
		if (hits[i].interceptPoint.y < center.y + length / 2 &&
			hits[i].interceptPoint.y > center.y - length / 2) { //find hit disk top bottom cylinder, closest T hit
			hit = hits[i];
			return;
		}
	}
...
/**
* @fn	void IClosedCylinderY::getTexCoords(const glm::vec3 &pt, float &u, float &v) const
* @brief	Gets tex coordinates
* @param 		  	pt	The point.
* @param [in,out]	u 	Tex coordinate u.
* @param [in,out]	v 	Tex coordinate v.
...
void IClosedCylinderY::getTexCoords(const glm::vec3 &pt, float &u, float &v) const {
