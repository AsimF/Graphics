 ...
 * @return	true iff (a-b) is in [-EPSILON, EPSILON]. (if their values are approximately close)
 */
...
glm::vec2 pointOnCircle(const glm::vec2 &center, float R, float angleRads) { //be able to write functions like this on exams.
...
//REMEMBER: atan2 (finds angle theta in RADIANS between opposite (y) over adjacent (x))
float directionInDegrees(float x1, float y1, float x2, float y2) {
	//float radius = sqrt(pow((x2 - x1),2) + pow((y2 - y1),2));
	return rad2deg(normalizeRadians(atan2(y2 - y1, x2 - x1)));
}
...
float directionInDegrees(const glm::vec2 &pt1, const glm::vec2 &pt2) {
	return rad2deg(normalizeRadians(atan2(pt2.y - pt1.y, pt2.x - pt1.x)));
}
...
float directionInDegrees(const glm::vec2 &targetPt) {
	return rad2deg(normalizeRadians(atan2(targetPt.y, targetPt.x)));
}
...
float map(float x, float xLow, float xHigh, float yLow, float yHigh) {
	float percent = (x - xLow) / (xHigh - xLow);
	float xPrime = ((yHigh - yLow) * percent) + yLow;
	return xPrime;
}
...
std::vector<float> quadratic(float A, float B, float C) {
	...
	float rootSm, rootBg, temp = 0;
	std::vector<float> result;
	if (discriminant < 0)
		return result;

	rootSm = (-B - sqrt(discriminant)) / (2 * A);
	rootBg = (-B + sqrt(discriminant)) / (2 * A);

	if (rootSm > rootBg && discriminant > 0) {
		temp = rootSm;
		rootSm = rootBg;
		rootBg = temp;
	}
	result.push_back(rootSm);

	if (discriminant > 0) {
		result.push_back(rootSm);
		result.push_back(rootBg);
	}
...
glm::vec3 getRow(const glm::mat3 &mat, int row) {
	return glm::vec3(mat[0][row], mat[1][row], mat[2][row]); //REMEMBER ==> Columns first, then rows! Also 0 index is the beginning!
}
